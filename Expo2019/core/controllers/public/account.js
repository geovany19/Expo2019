...
//toma el valor de la variable de la url token
...
$('#form-correo').submit(function()
{
    event.preventDefault();
    $.ajax({
        url: apiAccount + 'correoRecuperar',
        type: 'post',
...
{   event.preventDefault();
    ...
    console.log(token);
    var password1 = $('#clave_nueva_1').val(), password2 = $('#clave_nueva_2').val();
    $.ajax({
        url: apiAccount + 'recuperarContra',
        type: 'post',
        data: {
            token: token,
